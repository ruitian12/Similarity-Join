...
// static int bistr1[101];
// static int bistr2[101];
...
Datum jaccard_index(PG_FUNCTION_ARGS)
{
	...
	// memset(bistr1,0,sizeof(bistr1));
	// memset(bistr2,0,sizeof(bistr2));
    ...
    for(int i = 0; i <= l1; i ++){
        if(i == 0 || i == l1) countn ++;
            ...
            start1 = ref1[hash2c(s1[i - 1], s1[i])];
            if(start1 == 0) {ref1[hash2c(s1[i - 1], s1[i])] = i; countn ++;}
            else{
    ...
    for(int k = 0; k <= l2; k ++){
        if(k == 0){
            ...
            start1 = ref1[hash2c(s2[k - 1], s2[k])];
            start2 = ref2[hash2c(s2[k - 1], s2[k])];
            if(start2 == 0) {ref2[hash2c(s2[k - 1], s2[k])] = k;}
            else{
                ...
                if(j == 0){
                    index2[k - 1] = start2;
                    ref2[hash2c(s2[k - 1], s2[k])] = k;
                }
            }
            if(ref2[hash2c(s2[k - 1], s2[k])] == k){
                if(start1 != 0){
                    for(j = start1; ; j = index1[j - 1]){
                        if(j == 0) {
                            countn ++;
